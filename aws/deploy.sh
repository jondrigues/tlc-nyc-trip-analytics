...
echo "\nUploading lat_lon.csv file"
aws s3 cp $(pwd)/emr/lat_lon.csv $S3_EMR_BUCKET/aux_files/lat_lon.csv


echo "\nDownloading Postgres Jar"
curl https://jdbc.postgresql.org/download/postgresql-42.2.24.jar --output postgresql-42.2.24.jar
echo "\nUploading Postgres Jar"
aws s3 cp $(pwd)/postgresql-42.2.24.jar $S3_EMR_BUCKET/jars/postgresql-42.2.24.jar
echo "\nDeleting Postgres Jar"
rm $(pwd)/postgresql-42.2.24.jar
