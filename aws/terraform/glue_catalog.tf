...
resource "aws_glue_crawler" "refined_crawler" {
  ...
  s3_target {
    path = "${aws_glue_catalog_database.refined_layer.location_uri}/trip_data"
...
resource "aws_glue_crawler" "trusted_crawler" {
  ...
  s3_target {
    path = "${aws_glue_catalog_database.trusted_layer.location_uri}/green"
    connection_name = aws_glue_connection.s3_connection.name
  }

  s3_target {
    path = "${aws_glue_catalog_database.trusted_layer.location_uri}/yellow"
    connection_name = aws_glue_connection.s3_connection.name
  }
